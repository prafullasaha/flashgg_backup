...
public:
  ...
  void addDiPho_pt(double DiPho_pt)                                                         { DiPho_pt_      = DiPho_pt;  } ;
...
double THQ_BDT_Helper::EvalScore(const std::shared_ptr<BDT_Hadronic_tprime> object){
  ...
  if (show_input_variables) {
      //printf("%-20s: %.2f\n" , "maxIDMVA_"                 , maxIDMVA_                 );
      printf("%s: %8.7f,\n " , 	"diphoton_mass_					"            , object->diphoton->M()     );
      printf("%s: %8.7f,\n " ,  "diphoton_pt_                 			"              , object->diphoton->Pt()    );
      printf("%s: %8.7f,\n" , 	"[CHECK in Plugin]Through Add function DiPho_pt	"              ,DiPho_pt_	 	     );
      cout<<			"[CHECK in COUT]diphoton_pt in Plugin 		=" <<DiPho_pt_<<endl;  
